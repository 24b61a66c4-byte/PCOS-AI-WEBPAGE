...
class PCOSAnalyzer:
    ...
    def _calculate_risk_score(self, data: Dict, dataset: Dict) -> int:
        ...
        high_risk_symptoms = [
            'irregular_cycles', 'hirsutism', 'acne', 
            'weight_gain', 'hair_loss', 'infertility'
        ]
    ...
    def _create_summary(self, data: Dict, risk_level: str, cycle_status: str) -> str:
        ...
        if risk_level == "high":
            ...
            summary += "Continue monitoring your health and maintain a healthy lifestyle."
    ...
    def get_dataset_statistics(self) -> Dict[str, Any]:
        ...
        try:
            # Query dataset
            response = self.supabase.table('pcos_dataset_raw').select('*').limit(1000).execute()
            ...
            cycle_lengths = [int(d['cycle_length']) for d in data if d.get('cycle_length') and d['cycle_length'].isdigit()]
            period_lengths = [int(d['period_length']) for d in data if d.get('period_length') and d['period_length'].isdigit()]
            
            stats = {
                "total_entries": len(data),
                "avg_cycle_length": int(np.mean(cycle_lengths)) if cycle_lengths else 28,
                "avg_period_length": int(np.mean(period_lengths)) if period_lengths else 5,
    ...
    def _get_common_symptoms(self, data: List[Dict]) -> List[str]:
        ...
        symptom_fields = [
            'irregular_missed_periods', 'hair_growth_chin', 'acne_or_skin_tags',
            'weight_change', 'hair_thinning_or_hair_loss', 'always_tired'
        ]
