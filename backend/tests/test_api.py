...
class TestAnalyzeEndpoint:
    ...
    def test_analyze_returns_400_missing_age(self, client):
        ...
        response = client.post(
            "/api/analyze",
            data=json.dumps({"cycle_length": 28, "period_length": 5, "symptoms": []}),
            content_type="application/json",
        )
        assert response.status_code == 400
    ...
    def test_analyze_returns_400_missing_cycle_length(self, client):
        ...
        response = client.post(
            "/api/analyze",
            data=json.dumps({"age": 25, "period_length": 5, "symptoms": []}),
            content_type="application/json",
        )
        assert response.status_code == 400
    ...
    def test_analyze_returns_400_missing_period_length(self, client):
        ...
        response = client.post(
            "/api/analyze",
            data=json.dumps({"age": 25, "cycle_length": 28, "symptoms": []}),
            content_type="application/json",
        )
        assert response.status_code == 400
    ...
    def test_analyze_returns_400_missing_symptoms(self, client):
        ...
        response = client.post(
            "/api/analyze",
            data=json.dumps({"age": 25, "cycle_length": 28, "period_length": 5}),
            content_type="application/json",
        )
        assert response.status_code == 400
    ...
    def test_analyze_returns_200_valid_data(
        self, mock_save, mock_doctors, mock_analyze, client, mock_supabase
    ):
    ...
    def test_analyze_returns_analysis_result(
        ...
        response = client.post(
            "/api/analyze",
            data=json.dumps(
                {"age": 25, "cycle_length": 28, "period_length": 5, "symptoms": []}
            ),
            content_type="application/json",
        )
    ...
    @patch("app.doctor_recommender.get_recommendations")
    @patch("app.save_entry")
    def test_analyze_returns_doctors(
        self, mock_save, mock_doctors, mock_analyze, client, mock_supabase
    ):
        ...
        mock_doctors.return_value = [{"name": "Dr. Test", "specialty": "Gynecologist"}]
...
class TestErrorHandling:
    ...
    def test_invalid_json_returns_400(self, client):
        ...
        response = client.post(
            "/api/analyze", data="not valid json", content_type="application/json"
        )
